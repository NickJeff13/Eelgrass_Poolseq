...
bnam_raster_gen_season <- function(x,region=NULL,res_val=0.09,crs_val,rasterfun = min,funname="min"){
  ...
  for(i in 1:length(select_names)){
    ...
    for(j in c("winter","spring","summer","fall")){
      ...
      #file name for the geotiff
      filename <- paste0("output/bnam_rasters/",j,"_",funname,"_",paste0(gsub(".mat","",strsplit(x,"./")%>%unlist()%>%.[4]),".tif"))
