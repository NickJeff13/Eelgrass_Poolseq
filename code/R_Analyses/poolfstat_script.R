...
#Here we will start the poolfstat package script
#First set pool sizes vector and poolsizes (x2) for baypass
poolsizes = c(47,47,46,48,47,47,47,47,41,39,49,48,47,47,47,47,47,30,30,54,50,40,30)
poolsizesbaypass=poolsizes*2
poolnames = c("Pool1","Pool2","Pool3","Pool4","Pool5","Pool6","Pool7","Pool8","Pool9","Pool10","Pool11","Pool12","Pool13",
              "Pool14","Pool15","Pool16","Pool17","Pool18","Pool19","Pool20","Pool21","Pool22","Pool23")

#Let's read in our sync file for all pops generated by popoolation2 
#the sync file is 258 million lines, and this function reads in 1 million lines every 20 seconds or so based on a maf of 0.01

zospools.minrc4<-popsync2pooldata(sync.file = "ZosteraAllPools.sync",
                           poolsizes = c(47,47,46,48,47,47,47,47,41,39,49,48,47,47,47,47,47,30,30,54,50,40,30),
                           poolnames = c(paste0("Pool",1:23)),
                           min.rc = 4,
                           min.cov.per.pool = 20,
...
zospools.noTSW <- pooldata.subset(pooldata = zospools, 
                                  pool.index = 1:22,
                                  min.cov.per.pool = 20,
                                  max.cov.per.pool = 500,
                                  min.maf = 0.05,
...
zospools.forGV <- pooldata.subset(pooldata = zospools, 
                                  pool.index = c(1:15,17:22),
                                  min.cov.per.pool = 20,
                                  max.cov.per.pool = 500,
                                  min.maf = 0.05,
                                  verbose = T)
...
#Also make a subset of just Nova Scotia sites for fine-scale environmental associations
zospools.NS <- pooldata.subset(pooldata = zospools, 
                                  pool.index = c(1:3,6:7,12,16,22),
                                  min.cov.per.pool = 20,
                                  max.cov.per.pool = 500,
                                  min.maf = 0.05,
                                  verbose = T)
...
##################################################
zos.pair.fst.noTSW <- compute.pairwiseFST(zospools.noTSW, method = "Anova",
                                    min.cov.per.pool = 20, max.cov.per.pool = 500,
                                    min.maf = 0.01,
                                    output.snp.values = FALSE,
                                    verbose=TRUE)

#write the results to a csv file for use in other software
write.csv(zos.pair.fst.noTSW@PairwiseFSTmatrix,file="PairwiseFST.NoTSW.csv", quote=F)

#####This one includes TSW
...
#####Using our pooldata objects, we can also compute global Fst and per-snp Fst
#use a sliding window approach first
zos.snp.fsts <- computeFST(x=zospools,method = "Anova",sliding.window.size = 50)
...
#Let's also make a randomly subset BayPass file - this gave the same overall results as the full snp file
...
##################################################################################
#source this script from baypass wherever you installed it
...
###################################################################################################
###########################POPOOLATION2 ANALYSES##################################################
###################################################################################################

#Make some plots using the fst and other data from Popoolation2
library("qqman")
library("DataCombine")


setwd("/hdd3/EelgrassPoolSeq/trimmed/DeDuped/IndelRealigned/PopoolationResults/")
...
#Save the data
save.image("ZosteraPoolfstat.RData")
